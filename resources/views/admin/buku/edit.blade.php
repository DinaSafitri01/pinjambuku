...
<div class="animate__animated animate__fadeIn" style="max-width: 800px;">
    <div class="mb-4">
        ...
        <h2 class="fw-bold mt-2">Edit Data Buku</h2>
        <p class="text-muted">Perbarui informasi buku yang sudah ada di koleksi.</p>
    </div>
        ...
        <form action="{{ route('admin.buku.update', $buku->id) }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="row">
                <div class="col-12 mb-4">
                    ...
                    <div class="d-flex align-items-center gap-4">
                        <div id="image-preview" class="border rounded-4 d-flex align-items-center justify-content-center bg-light overflow-hidden shadow-sm" style="width: 140px; height: 180px; min-width: 140px;">
                            @if($buku->foto)
                                <img src="{{ asset('storage/books/' . $buku->foto) }}" style="width: 100%; height: 100%; object-fit: cover;">
                            @else
                        ...
                        <div class="flex-grow-1">
                            ...
                            <div class="form-text mt-2">
                                <i class="bi bi-info-circle me-1"></i> Biarkan kosong jika tidak ingin mengubah foto. Format: JPG, PNG. Maks 2MB.
                            </div>
                            @error('foto')
                ...
                <div class="col-md-12 mb-3">
                    ...
                    <input type="text" name="judul" class="form-control rounded-3 @error('judul') is-invalid @enderror" value="{{ old('judul', $buku->judul) }}" placeholder="Masukkan judul buku" required>
                    @error('judul')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="penulis" class="form-control rounded-3 @error('penulis') is-invalid @enderror" value="{{ old('penulis', $buku->penulis) }}" placeholder="Nama penulis" required>
                    @error('penulis')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="penerbit" class="form-control rounded-3 @error('penerbit') is-invalid @enderror" value="{{ old('penerbit', $buku->penerbit) }}" placeholder="Nama penerbit" required>
                    @error('penerbit')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="tahun_terbit" class="form-control rounded-3 @error('tahun_terbit') is-invalid @enderror" value="{{ old('tahun_terbit', $buku->tahun_terbit) }}" placeholder="Contoh: 2024" required>
                    @error('tahun_terbit')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="number" name="stok" class="form-control rounded-3 @error('stok') is-invalid @enderror" value="{{ old('stok', $buku->stok) }}" placeholder="Jumlah ketersediaan" required min="0">
                    @error('stok')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
